...
Phase 11 = product_status workflow (draft/pending_review/approved/published), TipTap rich-text editor, product creation form (/admin/products/new), review queue page, 3 new API routes (approve/reject/publish), migration 00009_product_workflow.sql.

Phase 12 = public product reviews with cookie-based own-pending visibility, admin moderation page, migration 00010_product_reviews.sql.

Phase 13 = Supabase Storage media manager (bucket `media`), folder via `.keep` placeholder convention, embedded MediaPickerDialog for product forms.
