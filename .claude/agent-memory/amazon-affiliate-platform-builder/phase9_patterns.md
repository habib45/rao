...
Phase 9 (April 2026) added the `import-product` and `publish-scheduled` Edge Functions, `publish_at` column on `products`, and dashboard widgets for both features.
...
- Supabase `functions.invoke<T>()` supports a generic return type — use it to type the expected Edge Function payload (e.g. `invoke<{ item: PAAPIItem }>`). Avoid unsafe casting.
- The thenable Supabase stub pattern (see `categories.test.ts`) does not work cleanly for routes that call `.functions.invoke(...)` AND `.from(...).upsert().select().single()` in the same request. Build a per-table `from` dispatcher (see `import.test.ts`) instead of the thenable chain.
