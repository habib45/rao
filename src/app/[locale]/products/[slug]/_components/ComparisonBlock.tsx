                ...
                {columns.map((col) => (
                  <th key={col.id} className="bg-surface p-4 min-w-40 text-center border-b border-border">
                    ...
                    <p className="text-left text-xs">
                      ...
                      <span className="font-semibold text-foreground">{col.title.length > 25 ? col.title.slice(0, 25) + "…" : col.title}</span>
                    </p>
                    {col.price && (
                      <p className="mt-0.5 text-left text-xs">
                        <span className="font-medium text-muted">Price: </span>
                        <span className="font-bold text-foreground">{col.price}</span>
                      </p>
                    )}
                    {col.rating != null && (
                      <p className="mt-0.5 text-left text-xs">
                        <span className="font-medium text-muted">Rating: </span>
                        <span className="text-amber-500">{"★".repeat(Math.round(col.rating))}{"☆".repeat(5 - Math.round(col.rating))}</span>
                        <span className="ml-1 font-semibold text-foreground">{col.rating.toFixed(1)}</span>
                      </p>
                    )}
                    {col.link && (
                      <a
                        ...
                        className="mt-2 block w-full rounded-md bg-brand px-2 py-0.75 text-center text-xs font-semibold text-white hover:bg-brand-dark transition-colors"
                      >
                        Buy on Amazon →
                      </a>
                    )}
            ...
            <div className="border-t border-border/50 bg-surface/30 py-3 text-center">
              ...
              >
                {expanded ? (
                  ...
                  <>See More ({rows.length - INITIAL_ROWS} more specs) <span className="text-base">∨</span></>
                )}
              </button>
            </div>
          )}
